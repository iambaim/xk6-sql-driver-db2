//go:build exclude
// Adapted from https://github.com/ibmdb/go_ibm_db/blob/master/installer/setup.go
...
func downloadFile(url string, filename string) error {
    ...
    out, err := os.Create("./helpers/" + filename)
    if err != nil {
...
func Unzipping(sourcefile string, targetDirectory string) {
    reader, err := zip.OpenReader("./helpers/" + sourcefile)
    if err != nil {
        fmt.Println(err)
        os.Exit(1)
    }
...
func linux_untar(clidriver string, targetDirectory string, ibmdbDir string) error {
    ...
    out, err := exec.Command("tar", "xzf", "./helpers/" + clidriver, "-C", targetDirectory).Output()
...
func aix_untar(clidriver string, targetDirectory string) error {
    ...
    gunzipOut, err := exec.Command("gunzip", "./helpers/"+clidriver).Output()
    ...
    tarOut, err := exec.Command("tar", "xvf", "./helpers/"+clidriver, "-C", targetDirectory).Output()
